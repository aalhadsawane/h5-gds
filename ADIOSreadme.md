...
2.  **Verify Installation**:
    ...
    The script generates `adios2_env.sh` in the project root which contains the necessary environment variables.
...
1.  **Load Environment**:
    ...
    # Load ADIOS2 environment variables generated by build script
    # (Consider moving this file to $HOME/env/ for portability)
    source adios2_env.sh
    ```

2.  **Configure**:
    Run `build.sh`. This script will automatically download CUDA samples if they are not found.

    ```bash
    ./build.sh
    ```

    *If you have existing CUDA samples you wish to use:*
    ```bash
